...
export const Field = (props) => {
    const { name, initialValue, onChange = () => void 0, onFocus = () => void 0, onBlur = () => void 0, transformInput = input => input, transformOutput = output => output, validateOnChange, children, validate = getValidity, } = props;
    ...
    useEffect(() => {
        if (!didMountRef.current || _validateOnChange || !changed)
            return;
        setDirty(true);
        setPristine(false);
    ...
    const _onBlur = event => {
        onBlur(event);
        if (!formState.validateOnBlur || !changed)
            return;
        setDirty(true);
        setPristine(false);
        setValidity(validate(value));
